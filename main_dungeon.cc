...
void char_move(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, int ch, Csr &csr_pos, const std::vector<monster> &monsters, Player &User, NPC &npc, level Current){
    bool require_move=false;
    if((ch=='a'||ch==SDLK_LEFT)&&csr_pos.first>1&&!check_surroundings(monsters, csr_pos.first-1, csr_pos.second)){
        TCOD_console_put_char_ex(mainWin.get(), csr_pos.first, csr_pos.second+1, ' ', WHITE, BLACK);
        csr_pos.first--;
        require_move=true;
    }
    ...
    if(require_move){
        ...
        draw_doors(mainWin, context, Current);
...
bool player_battle(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Player &User, level Current, Dungeon monster_type){
    ...
    while(true){
        ...
        tcod::print(*mainWin, {1, 49}, user_output.str(), &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
        tcod::print(*mainWin, {1, 2}, enemy_output.str(), &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
        tcod::print(*mainWin, {39, 25}, log.first, &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_CENTER);
        tcod::print(*mainWin, {39, 26}, log.second, &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_CENTER);
        context->present(*mainWin);
        ...
        if(ch=='1'){
            ...
            if(monster.hp<=0){
                ...
                SDL_wclear_mainWin(mainWin, context);
                tcod::print(*mainWin, {0, 1}, "Press any key to keep and [r] to trash", &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
                draw_stats(mainWin, context, User);
                print_description(mainWin, context, &loot, 1);
...
std::pair<bool, bool> attack_monster(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, std::vector<monster> &monsters, Csr csr_pos, Player &User, level Current){
    ...
    if(check_surroundings(monsters, csr_pos.first-1, csr_pos.second)){
        ...
        attack_status.second=player_battle(mainWin, context, User, Current, pos.second);
...
void print_food(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Player &User){
    SDL_wclear_mainWin(mainWin, context);
    std::stringstream ss;
    ss << "[1] Bread (30 Saturation Points): " << User.inv.food.bread;
    tcod::print(*mainWin, {0, 1}, ss.str(), &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    ss.str(std::string());
    ss << "[2] Waffle (50 Saturation Points): " << User.inv.food.waffle;
    tcod::print(*mainWin, {0, 2}, ss.str(), &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    ss.str(std::string());
    ...
    tcod::print(*mainWin, {0, 4}, ss.str(), &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    ss.str(std::string());
...
void eat_drink_mode(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Player &User){
    int ch;
    while(true){
        print_food(mainWin, context, User);
        draw_stats(mainWin, context, User);
        ch=SDL_getch(mainWin, context);
...
void main_dungeon(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Csr &csr_pos, Player &User, level &Current, std::vector<monster> &monsters, NPC &npc, NoDelete &perm_config){
    std::vector<std::pair<int, int>> doors;
    if(monsters.empty()){
        generate_monsters(monsters, Current, csr_pos);
    }
    generate_doors(doors, Current);
    redraw_everything(mainWin, context, csr_pos, User, Current, monsters);
    int ch;
    while(true){
        ch=SDL_getch(mainWin, context);
        if(ch=='w'||ch=='a'||ch=='s'||ch=='d'||ch==SDLK_LEFT||ch==SDLK_RIGHT||ch==SDLK_DOWN||ch==SDLK_UP){
            char_move(mainWin, context, ch, csr_pos, monsters, User, npc, Current);
        }
        else if(ch=='z'){
            ...
            std::pair<bool, bool> attack_status=attack_monster(mainWin, context, monsters, csr_pos, User, Current);
            ...
            if(move_staircase(Current, csr_pos)){
                ...
                bar_mode(mainWin, context, User, npc, perm_config);
            }
            redraw_everything(mainWin, context, csr_pos, User, Current, monsters);
        }
        else if(ch=='i'){
            ...
            save_data(User, Current, csr_pos, monsters, npc, perm_config);
            redraw_everything(mainWin, context, csr_pos, User, Current, monsters);
            clear_and_draw_dialog(mainWin, context, "Data saved successfully!");
        }
        else if(ch=='H'){
            help_mode(mainWin, context, "dungeon_mode");
            redraw_everything(mainWin, context, csr_pos, User, Current, monsters);
        }
        else if(ch=='q'){
            if(SDL_getch_y_or_n(mainWin, context, "Do you really wish to quit? [y] to quit, any other key to abort.")){
                end_program(0);
                save_data(User, Current, csr_pos, monsters, npc, perm_config);
                return;
            }
