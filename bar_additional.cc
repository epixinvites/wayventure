...
void print_item_description_1(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const Item *cur_item, int &line){
...
void print_item_description_2(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const Item *cur_item, int &line){
    std::stringstream ss;
...
void print_trader_item_description(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const Item *cur_item, int line, int price, bool is_locked){
    if(is_locked){
        tcod::print(*mainWin, {0, line+1}, "[Locked]", &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
        line++;
    }
    print_item_description_1(mainWin, context, cur_item, line);
    tcod::print(*mainWin, {0, line+1}, "Price: "+std::to_string(price), &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    line++;
    print_item_description_2(mainWin, context, cur_item, line);
    context->present(*mainWin);
}

void print_mysterious_trader_item_description(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const Item *cur_item, int line, double quality_points){
    print_item_description_1(mainWin, context, cur_item, line);
    tcod::print(*mainWin, {0, line+1}, "Quality Points: "+std::to_string(quality_points), &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    line++;
    print_item_description_2(mainWin, context, cur_item, line);
    context->present(*mainWin);
}

void print_sell_item_description(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const Item *cur_item, int line, int price){
    print_item_description_1(mainWin, context, cur_item, line);
...
void draw_trader_items(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const Player &User, const std::vector<Item *> &items_copy, unsigned int page_num, unsigned int csr_pos, int price, bool is_locked=false){
    SDL_wclear_mainWin(mainWin, context);
    SDL_wclear_stats_bar(mainWin, context);
    draw_stats(mainWin, context, User);
    draw_base(mainWin, context, 34, items_copy.size(), page_num, false, false);
    ...
    print_trader_item_description(mainWin, context, items_copy[page_num*30+csr_pos], 35, price, is_locked);
}

void draw_mysterious_trader_items(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const std::vector<Item *> &items_copy, unsigned int csr_pos, double quality_points){
    SDL_wclear_mainWin(mainWin, context);
    SDL_wclear_stats_bar(mainWin, context);
    draw_base(mainWin, context, 34, items_copy.size(), 0, false, false);
    for(int i=0, iterator=0; i<items_copy.size()&&iterator<30; i++, iterator++){
        print_item(mainWin, context, items_copy[i], iterator, csr_pos==iterator);
    }
    print_mysterious_trader_item_description(mainWin, context, items_copy[csr_pos], 35, quality_points);
}

void draw_sell_items(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const Player &User, const std::vector<Item *> &items_copy, unsigned int page_num, unsigned int csr_pos, int price){
    SDL_wclear_mainWin(mainWin, context);
    SDL_wclear_stats_bar(mainWin, context);
    draw_stats(mainWin, context, User);
    draw_base(mainWin, context, 34, items_copy.size(), page_num, false, false);
    ...
    print_sell_item_description(mainWin, context, items_copy[page_num*30+csr_pos], 35, price);
}

void store_misc_items(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Miscellaneous &User, Chest &chest){
    unsigned int csr_pos=0;
    clear_and_draw_dialog(mainWin, context, "[Inventory] Miscallaneous items:");
    int ch;
    while(true){
        ...
        if((ch==SDLK_DOWN||ch=='s')&&csr_pos<16){
            ...
            unsigned int quantity=get_int(mainWin, context, "([ESC] to cancel) Quantity to be stored: ");
...
void retrieve_misc_items(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Miscellaneous &User, Chest &chest){
    unsigned int csr_pos=0;
    clear_and_draw_dialog(mainWin, context, "[Storage] Miscallaneous items:");
    int ch;
    while(true){
        print_misc_item(mainWin, context, chest.misc_storage, csr_pos);
        ch=SDL_getch(mainWin, context);
        if((ch==SDLK_DOWN||ch=='s')&&csr_pos<16){
            clear_and_draw_dialog(mainWin, context, "[Storage] Miscallaneous items:");
            csr_pos++;
            ...
            unsigned int quantity=get_int(mainWin, context, "([ESC] to cancel) Quantity to be retrieved: ");
...
void inventory_storage(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Player &User, NoDelete &perm_config, Chest &chest){
    ...
    while(true){
        ...
        if((ch=='s'||ch==SDLK_DOWN)&&((csr_pos+page_num*30)<items_copy.size()-1&&csr_pos<29)){
            ...
            if(unequip_item(User, items_copy[csr_pos+page_num*30])){
                ...
                clear_and_draw_dialog(mainWin, context, "[System] Error Storing Item");
            }
        }
        else if(ch=='q'){
            return;
...
void inventory_retrieve(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Player &User, NoDelete &perm_config, Chest &chest){
    std::vector<Item *> items_copy;
    unsigned int page_num=0;
    int csr_pos=0;
    bool is_modifier_called=false;
    ...
    while(true){
        ...
        if((ch=='s'||ch==SDLK_DOWN)&&((csr_pos+page_num*30)<items_copy.size()-1&&csr_pos<29)){
            ...
            is_modifier_called=true;
            inventory_modifier_selection(mainWin, context, perm_config, chest.gear_storage, items_copy);
            process_copy(chest.gear_storage, items_copy, perm_config);
            draw_inventory(mainWin, context, User, items_copy, page_num, csr_pos);
        }
    }
...
void show_merchant_items(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Merchant &gear_merchant, Player &User){
    unsigned int page_num=0;
    int csr_pos=0;
    ...
    int ch;
    draw_trader_items(mainWin, context, User, items_copy, page_num, csr_pos, calculate_price(original_copy[csr_pos+page_num*30], gear_merchant.relation));
    while(true){
        ch=SDL_getch(mainWin, context);
        if((ch=='s'||ch==SDLK_DOWN)&&((csr_pos+page_num*30)<items_copy.size()-1&&csr_pos<29)){
            csr_pos++;
            tcod::print(*mainWin, {0, 39}, empty_line, &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
            draw_trader_items(mainWin, context, User, items_copy, page_num, csr_pos, calculate_price(original_copy[csr_pos+page_num*30], gear_merchant.relation), gear_merchant.store[csr_pos+page_num*30].second);
        }
        else if((ch=='w'||ch==SDLK_UP)&&csr_pos>0){
            csr_pos--;
            tcod::print(*mainWin, {0, 39}, empty_line, &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
            ...
            if(!gear_merchant.store[csr_pos+page_num*30].second){
                ...
                draw_trader_items(mainWin, context, User, items_copy, page_num, csr_pos, calculate_price(original_copy[csr_pos+page_num*30], gear_merchant.relation), gear_merchant.store[csr_pos+page_num*30].second);
                clear_and_draw_dialog(mainWin, context, "Item Locked!");
            }
            else{
                gear_merchant.store[csr_pos+page_num*30].second=false;
                draw_trader_items(mainWin, context, User, items_copy, page_num, csr_pos, calculate_price(original_copy[csr_pos+page_num*30], gear_merchant.relation), gear_merchant.store[csr_pos+page_num*30].second);
                clear_and_draw_dialog(mainWin, context, "Item Unlocked!");
            }
        }
        else if(ch=='B'){
            if(SDL_getch_y_or_n(mainWin, context, "Confirm purchase of item? [y/n]")){
                ...
                if(amount<=User.gold){
                    ...
                    init_copy(original_copy, items_copy);
                    draw_trader_items(mainWin, context, User, items_copy, page_num, csr_pos, calculate_price(original_copy[csr_pos+page_num*30], gear_merchant.relation), gear_merchant.store[csr_pos+page_num*30].second);
                    clear_and_draw_dialog(mainWin, context, "[System] Item added to inventory");
                }
                else{
                ...
                clear_and_draw_dialog(mainWin, context, "[System] Transaction aborted");
            }
        }
        else if(ch=='q'){
            return;
...
void sell_items_to_merchant(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Merchant &gear_merchant, Player &User, NoDelete &perm_config){
    std::vector<Item *> items_copy;
    unsigned int page_num=0;
    int csr_pos=0;
    ...
    int ch;
    draw_sell_items(mainWin, context, User, items_copy, page_num, csr_pos, calculate_price(User.inv.item[csr_pos+page_num*30], gear_merchant.relation, true));
    while(true){
        ch=SDL_getch(mainWin, context);
        if((ch=='s'||ch==SDLK_DOWN)&&((csr_pos+page_num*30)<items_copy.size()-1&&csr_pos<29)){
            csr_pos++;
            draw_sell_items(mainWin, context, User, items_copy, page_num, csr_pos, calculate_price(*items_copy[csr_pos+page_num*30], gear_merchant.relation, true));
        }
        else if((ch=='w'||ch==SDLK_UP)&&csr_pos>0){
            csr_pos--;
            draw_sell_items(mainWin, context, User, items_copy, page_num, csr_pos, calculate_price(*items_copy[csr_pos+page_num*30], gear_merchant.relation, true));
            ...
            if(SDL_getch_y_or_n(mainWin, context, "Sell item? [y/n]")&&unequip_item(User, items_copy[csr_pos+page_num*30])){
...
void print_trader_misc_menu(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, unsigned int csr_pos){
    SDL_wclear_mainWin(mainWin, context);
    // Buy
    tcod::print(*mainWin, {0, 1}, "Buy Materials/Blueprints:", &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    print_bold_with_condition(mainWin, context, "[Common] "+std::to_string(trader_prices.at("common_material"))+" Gold each", WHITE, 2, csr_pos==0);
    print_bold_with_condition(mainWin, context, "[Uncommon] "+std::to_string(trader_prices.at("uncommon_material"))+" Gold each", GREEN, 3, csr_pos==1);
    print_bold_with_condition(mainWin, context, "[Rare] "+std::to_string(trader_prices.at("rare_material"))+" Gold each", BLUE, 4, csr_pos==2);
    print_bold_with_condition(mainWin, context, "[Epic] "+std::to_string(trader_prices.at("epic_material"))+" Gold each", PURPLE, 5, csr_pos==3);
    print_bold_with_condition(mainWin, context, "[Legendary] "+std::to_string(trader_prices.at("legendary_material"))+" Gold each", YELLOW, 6, csr_pos==4);
    tcod::print(*mainWin, {0, 8}, "Blueprints:", &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    print_bold_with_condition(mainWin, context, "[Helmet] "+std::to_string(trader_prices.at("helmet_blueprint"))+" Crystal Cores each", WHITE, 9, csr_pos==5);
    print_bold_with_condition(mainWin, context, "[Chestplate] "+std::to_string(trader_prices.at("chestplate_blueprint"))+" Crystal Cores each", WHITE, 10, csr_pos==6);
    print_bold_with_condition(mainWin, context, "[Greaves] "+std::to_string(trader_prices.at("greaves_blueprint"))+" Crystal Cores each", WHITE, 11, csr_pos==7);
    print_bold_with_condition(mainWin, context, "[Boots] "+std::to_string(trader_prices.at("boots_blueprint"))+" Crystal Cores each", WHITE, 12, csr_pos==8);
    print_bold_with_condition(mainWin, context, "[Shield] "+std::to_string(trader_prices.at("shield_blueprint"))+" Crystal Cores each", WHITE, 13, csr_pos==9);
    print_bold_with_condition(mainWin, context, "[Weapon] "+std::to_string(trader_prices.at("weapon_blueprint"))+" Crystal Cores each", WHITE, 14, csr_pos==10);
    tcod::print(*mainWin, {0, 16}, "Others:", &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    print_bold_with_condition(mainWin, context, "[First-Aid Kits] "+std::to_string(trader_prices.at("first_aid_kit"))+" Crystal Cores each", PURPLE, 17, csr_pos==11);
    // Sell
    tcod::print(*mainWin, {0, 21}, "--------------------------------------------------------------------------------", &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    tcod::print(*mainWin, {0, 25}, "Sell Cores/Materials:", &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    tcod::print(*mainWin, {0, 26}, "Cores:", &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    print_bold_with_condition(mainWin, context, "[Crystal Cores] "+std::to_string((unsigned long long int) (trader_prices.at("crystal_core")*0.8))+" Gold each", BLUE, 27, csr_pos==12);
    tcod::print(*mainWin, {0, 29}, "Materials:", &WHITE, &BLACK, TCOD_BKGND_SET, TCOD_LEFT);
    print_bold_with_condition(mainWin, context, "[Common] "+std::to_string((unsigned long long int) (trader_prices.at("common_material")*0.8))+" Gold each", WHITE, 30, csr_pos==13);
    print_bold_with_condition(mainWin, context, "[Uncommon] "+std::to_string((unsigned long long int) (trader_prices.at("uncommon_material")*0.8))+" Gold each", GREEN, 31, csr_pos==14);
    print_bold_with_condition(mainWin, context, "[Rare] "+std::to_string((unsigned long long int) (trader_prices.at("rare_material")*0.8))+" Gold each", BLUE, 32, csr_pos==15);
    print_bold_with_condition(mainWin, context, "[Epic] "+std::to_string((unsigned long long int) (trader_prices.at("epic_material")*0.8))+" Gold each", PURPLE, 33, csr_pos==16);
    print_bold_with_condition(mainWin, context, "[Legendary] "+std::to_string((unsigned long long int) (trader_prices.at("legendary_material")*0.8))+" Gold each", YELLOW, 34, csr_pos==17);
    clear_and_draw_dialog(mainWin, context, "Material Shop:");
}
...
bool trader_process_payment(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Player &User, Merchant &gear_merchant, unsigned int csr_pos){
    unsigned long long int amount=0;
    if(csr_pos<12){
        amount=get_ullint(mainWin, context, "Please enter the amount of items you wish to purchase: ");
    }
    else if(csr_pos<18){
        amount=get_ullint(mainWin, context, "Please enter the amount of items you wish to sell: ");
    }
    if(amount<=0){
        return false;
    }
...
void trader_misc_menu(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Player &User, Merchant &gear_merchant){
    unsigned int csr_pos=0;
    int ch;
    print_trader_misc_menu(mainWin, context, csr_pos);
    while(true){
        ch=SDL_getch(mainWin, context);
        if((ch=='w'||ch==SDLK_UP)&&csr_pos>0){
            ...
            if(trader_process_payment(mainWin, context, User, gear_merchant, csr_pos)){
                print_trader_misc_menu(mainWin, context, csr_pos);
                draw_stats(mainWin, context, User);
                clear_and_draw_dialog(mainWin, context, "[System] Success");
            }
...
void print_inventory_with_quality_points_description(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const Item *cur_item, int line){
    print_item_description_1(mainWin, context, cur_item, line);
...
void print_item_with_selection(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, const Item *cur_item, int line, bool is_highlighted, bool is_selected){
    std::stringstream ss;
    if(is_selected){
        ss << "->";
    }
    ...
    print_bold_with_condition(mainWin, context, ss.str(), YELLOW, line+1, is_highlighted);
}

void draw_base_with_quality_points(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, int y, unsigned int size, unsigned int page, const int &target_quality, const int &total_quality){
    std::stringstream ss;
...
void print_inventory_with_quality_points(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, unsigned int csr_pos, unsigned int page_num, const Player &User, const std::vector<std::pair<Item *, bool>> &items_copy, const int &target_quality, const int &total_quality){
    SDL_wclear_mainWin(mainWin, context);
    SDL_wclear_stats_bar(mainWin, context);
    draw_stats(mainWin, context, User);
    draw_base_with_quality_points(mainWin, context, 34, items_copy.size(), page_num, target_quality, total_quality);
    for(long long int i=page_num*30, iterator=0; i<items_copy.size()&&iterator<30; i++, iterator++){
        print_item_with_selection(mainWin, context, items_copy[i].first, iterator, csr_pos==iterator, items_copy[page_num*30+iterator].second);
    }
    print_inventory_with_quality_points_description(mainWin, context, items_copy[page_num*30+csr_pos].first, 35);
}

int select_inventory_for_exchange(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Player &User, const Item *target){
    ...
    int ch;
    print_inventory_with_quality_points(mainWin, context, csr_pos, page_num, User, selected_items, calculate_quality_points(*target), total_quality);
    while(true){
        ch=SDL_getch(mainWin, context);
...
void mysterious_trader_items(tcod::ConsolePtr &mainWin, tcod::ContextPtr &context, Player &User, Merchant &mysterious_merchant){
    unsigned int page_num=0;
    int csr_pos=0;
    std::vector<Item> original_copy;
    ...
    init_copy(original_copy, items_copy);
    int ch;
    draw_mysterious_trader_items(mainWin, context, items_copy, csr_pos, calculate_quality_points(*items_copy[csr_pos]));
    while(true){
        ch=SDL_getch(mainWin, context);
        if((ch=='s'||ch==SDLK_DOWN)&&((csr_pos+page_num*30)<items_copy.size()-1&&csr_pos<29)){
            csr_pos++;
            ...
            for(int i=0; i<User.inv.item.size(); i++){
                if(User.inv.item[i].rarity==Rarity::LEGENDARY){
                    int status=select_inventory_for_exchange(mainWin, context, User, &mysterious_merchant.store[csr_pos].first);
                    if(status==1){
                        draw_mysterious_trader_items(mainWin, context, items_copy, csr_pos, calculate_quality_points(*items_copy[csr_pos]));
                        clear_and_draw_dialog(mainWin, context, "[Trader] ...*Stares*");
                    }
                    else if(status==2){
                        ...
                        csr_pos=0;
                        draw_mysterious_trader_items(mainWin, context, items_copy, csr_pos, calculate_quality_points(*items_copy[csr_pos]));
                        clear_and_draw_dialog(mainWin, context, "[Trader] Let fate intertwine our paths once more...");
                    }
                    else{
                        draw_mysterious_trader_items(mainWin, context, items_copy, csr_pos, calculate_quality_points(*items_copy[csr_pos]));
                    }
