/*
 * Copyright (C) 2022  Zhi Ping, Ooi <epixunknown@protonmail.com>
 * Author: Zhi Ping, Ooi <epixunknown@protonmail.com>
 *
...
int SDL_getch(tcod::Console& main_win, tcod::ContextPtr& context) {
	...
	while(SDL_PollEvent(&event)) {
		if(event.type == SDL_WINDOWEVENT) {
			if(event.window.event == SDL_WINDOWEVENT_EXPOSED) { context->present(main_win); }
		}
		...
		if(event.type == SDL_KEYDOWN) {
			if(event.key.keysym.mod & KMOD_SHIFT) {
				...
				} catch(const std::out_of_range) { return event.key.keysym.sym; }
			}
...
void SDL_wclear_stats_bar(tcod::Console& main_win, tcod::ContextPtr& context) {
	for(int i = 51; i < 56; i++) { tcod::print(main_win, {0, i}, empty_line, WHITE, BLACK); }
	//    context->present(main_win);
}

void SDL_wclear_main_win(tcod::Console& main_win, tcod::ContextPtr& context) {
	for(int i = 1; i < 51; i++) {
		for(int j = 0; j < 80; j++) { tcod::print(main_win, {0, i}, empty_line, WHITE, BLACK); }
	}
	//    context->present(main_win);
}
...
void end_program(int sig) {
	if(sig == 0) {
		...
		std::string line;
		while(std::getline(cosmos_ascii, line)) { std::cout << line << std::endl; }
		return;
	}
	if(sig == 1) {
		std::ifstream tombstone_file("src/res/tombstone.txt");
		std::string line;
		while(std::getline(tombstone_file, line)) { std::cout << line << std::endl; }
		return;
	}
	if(sig == 2) {
	...
	if(sig == 3) { std::cout << "*Cough* *Cough* You died of thirst... Next time, remember to join the HydroHomies. (Remember to drink water bro)" << std::endl; }
}
...
void init_data(Dungeon& dungeon_data, Player& user, No_Delete& perm_config) {
	...
	if(username.length() > 30) { throw std::runtime_error("Nope. Nopenopenopenope. You didn't follow my instructions."); }
	...
	if(std::filesystem::exists(std::filesystem::path("save/user.save"))) {
		...
		if(save_file_version != version_check) { throw std::runtime_error("Versions of save files do not match. Aborting. Do not attempt to edit the save files, lost of data will be expected."); }
		user.init();
...
void starting_screen(tcod::Console& main_win, tcod::ContextPtr& context, Player& user, Dungeon& dungeon_data, No_Delete& perm_config) {
	...
	std::string line;
	while(std::getline(ascii_wayfarer, line)) { logo.push_back(line); }
